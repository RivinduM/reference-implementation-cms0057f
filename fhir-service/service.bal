...
isolated function invokePatientExport(string patientId, map<string[]> queryParameters, fhirClient:RequestMode requestMode) returns r4:FHIRError|http:Response|error {
    ...
    if exportResponse is fhirClient:FHIRError {
        ...
        return r4:createFHIRError(exportResponse.message(), r4:ERROR, r4:PROCESSING, httpStatusCode = statusCode);
    }
...
service /fhir/r4/Patient on new fhirr4:Listener(config = patientApiConfig) {
    ...
    isolated resource function get [string id]/\$export(r4:FHIRContext fhirContext) returns r4:FHIRError|http:Response|error {
        ...
        if consentContext !is () {
            if consentContext.patientID != id {
                return r4:createFHIRError("Requested patient id does not match consent context patient id", r4:ERROR,
...
service /fhir/r4/Claim on new fhirr4:Listener(config = ClaimApiConfig) {
    ...
    isolated resource function post \$submit\-attachment(r4:FHIRContext fhirContext, Parameters parameters) returns r4:OperationOutcome|r4:Bundle|error {
        // update claim persisting attachments
        r4:FHIRError|r4:Bundle|error claimSubmissionResponse = submitAttachments(parameters);
        // update communication request status to "completed" if claim submission is successful
        if claimSubmissionResponse is r4:Bundle {
            return updateCommunicationRequestStatus(parameters);
        }
